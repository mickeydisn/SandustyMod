...
**Pitch:** Sealed barrels that _age_ elements using `setDurationAtCell` + long processing. Wet sand
aged 60s becomes a custom `vintageSlurry` with higher collector value. Interact to “uncork.”

**Why it’s not trivial:** Time-gated transform, custom element register, discoveries on first
uncork, locked interactable while aging, lights that warm with age.

**APIs:** elements, structures, processing, signals.interactables, discoveries, collector-related
flags, time, lights, i18n
