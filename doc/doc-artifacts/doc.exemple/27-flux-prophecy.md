...
**Pitch:** Shrine consumes energy bids from linked batteries. Every 60s a winner channel is rolled
(`random` + shared buffer). Winning signal line pulses hard; losers get smoke. Jackpot event grants
temporary process rate toast / fake “blessing.”

**Why it’s not trivial:** Multi-structure ritual, energy.consume, shared buffer ceremony state
machine, dramatic lights/camera, social nonsense in singleplayer.

**APIs:** structures, energy, signals, random, shared.buffers, triggers, lights, camera, sound, ui,
events
