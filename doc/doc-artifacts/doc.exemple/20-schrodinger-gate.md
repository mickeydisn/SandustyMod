...
**Pitch:** Each tick the gate randomly _collapses_ to ALLOW or BLOCK. Looking at it (lens scan /
tooltip open) freezes the state. Signal receivers see flickering truth.

**Why it’s not trivial:** Uses `random`, `signals` sender that changes every pulse, `events` from
Prospector’s Lens to “observe”, `structures.updateData` for collapsed flag, UI toast “Collapsed:
ALLOW”. Comedy + actual logistics chaos.
