...
**APIs:** blueprints, building, authorization, structures, input, ui, storage, events, action,
camera

**Idea:** Tool mode that copies a structure rectangle if `canBuildAtCell` passes for every cell,
serializes via blueprints API, stores in mod storage, and pastes with authorization checks + toast
UI.
