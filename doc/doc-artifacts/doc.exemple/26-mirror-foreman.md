...
**Pitch:** Set a mirror axis structure. Whenever you `buildAtCell` a whitelisted type on one side,
the foreman mirrors it across the axis if `canBuildAtCell` allows. Demolish mirrors too.

**Why it’s not trivial:** Hooks/events on place, authorization double-check, axis placement config,
prevents infinite recursion with a re-entrancy flag in shared buffer.
...
export async function init({ api }) {
    ...
    api.events.on("weird.mirror:build", ({ x, y, typeId }) => {
        ...
        if (buf[0]) return;
        let axisX = null;
        api.structures.forEachOfType(AXIS, (a) => {
            axisX = a.x;
        });
