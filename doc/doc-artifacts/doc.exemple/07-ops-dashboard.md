...
**APIs:** ui, events, triggers, structures, energy, factory, rendering, input, storage, i18n,
settings

**Idea:** Pause-menu style panel + overlay that aggregates events from Ex 01–06: state-pad changes,
reactor heat, blasts, alchemy production, battery capacity.
...
export async function init(sandkit) {
    ...
    api.events.on("myMod.statePad:changed", () => {
        stats.padFlips++;
        save();
    });
    api.events.on("myMod.reactor:overheat", () => {
        stats.overheats++;
        save();
    });
    api.events.on("myMod.terraformer:blast", () => {
        stats.blasts++;
        save();
    });
    api.events.on("myMod.alchemy:produced", () => {
        stats.dust++;
        save();
    });
