...
**APIs:** items, sprites, input, raycast, patterns, excavation, terrains, elements, grid, cooldown,
energy, resources, sound, i18n

**Idea:** Hold tool, press binding, raycast to a cell, dig a circle, convert terrain to sand, spend
energy, play positional sound.
...
function fire(api, cooldown) {
    ...
    const ok = api.resources.adjustEnergy ? (api.energy.consume?.(20) ?? true) : true;
    if (!ok) {
