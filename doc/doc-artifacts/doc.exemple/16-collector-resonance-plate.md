...
**Vibe:** Collectors are silent squares. This plate under/near collectors **sings** when gold value
is present, shows ring light intensity by `collector.getValue*`, and optional toast when gold is
spent (value drops).
...
Gold is _the_ research currency. Players want audiovisual confirmation that Collectors are “live”
without opening UI.
    ...
    api.structures.processing.register(`${SID}:hum`, {
        ...
        process: (s) => {
            ...
            if (value < prev - 5) {
                ...
                api.sound.play(
                    "ui.coins",
                    api.sound.calculateDistanceOptionsAtWorld(s.x * 4, s.y * 4, 1),
                );
            }
