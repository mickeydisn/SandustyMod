...
**Pitch:** A rare global event (every N minutes, or forced by shrine). During blood moon: `hooks`
amplify fire, `reactions` path feels different (temp register), sky lights tint via many temporary
lights on player, maps markers, ambient sound layers. Ends cleanly.

**Why it’s not trivial:** Timed global state in shared buffer, multi-system takeover (fire, lights,
sound, hooks, events), cinematic camera, safe cleanup — a _director_, not a building.
...
**Player fantasy:** The planet itself becomes a character; other mods can react to
`bloodMoon:start`.
