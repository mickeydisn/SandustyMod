...
Wireable structure signals: player **interact** (click/use), **senders**, **receivers**, and link
state.
...
Engine also has internal link helpers (`set`/`setAll`/`getCombinedAt`/…) used by vanilla logic
structures.
...
**How it runs:** Engine intercepts `action:intercept` when the player uses the cell under the
cursor. Skips demolish, marquee, and signal-linker modes. Looks up handler by `structure.type`,
invokes it, cancels default action. Also drives hover highlight unless `data.locked` or
`interactable:suppressHover` intercept blocks it.
...
4. Interactables respond to use; targets respond to graph updates; senders publish via
   `setOutputAtCell` / `getOutput`
