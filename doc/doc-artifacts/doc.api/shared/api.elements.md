...
> **Object shapes:**
> [definitions/api.elements.definition.md](../definitions/api.elements.definition.md)
...
**Why Main only:** Type tables and UI pickers live on the main runtime. Worker only resolves
already-known types via `getTypeById`.
...
Replace ignores empty checks differently than create. Remove no-ops if no resolved element at cell
(worker).
...
1. Prefer string ids (`"water"`) when official examples do; resolve with `getTypeById` when you need
   numeric types.
