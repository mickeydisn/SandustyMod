 ...
 */
import type { ForceConfig } from "@sandmd/element-profiles";
...
export function createDefaultState(): Record<string, boolean | number> {
    const state: Record<string, boolean | number> = {};
    for (const f of CONFIG_FIELDS) state[f.key] = f.default;
    return state;
}
...
/** Direction labels offered in the panel (mirrors DirectionName + "sides"). */
export const FORCE_DIRECTIONS: readonly string[] = [
    "sides",
    "top",
    "bottom",
    "left",
