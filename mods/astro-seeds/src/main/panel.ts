...
// Bound up-front; `mountPanel` guards against a missing React host before any
// render, so callers can rely on `h` being callable.
const h = React?.createElement.bind(React) as ReactLike["createElement"];
...
function inGame(): boolean {
    ...
    const Scene =
        safe(() => (sandkit.enums as unknown as { Scene?: Record<string, number> })?.Scene) || {};
