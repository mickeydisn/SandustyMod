/**
 * Hidden World — seeded simplex noise.
 *
 * Minimal 2D simplex-noise implementation seeded the same way the web
 * generator uses it (`new SimplexNoise(seed)` with a string/number seed).
 * Ported so the mod has no CDN / external dependency.
 */

/** xmur3 string hash → 32-bit seed (same trick as the simplex-noise lib). */
...
export class SimplexNoise {
    ...
    constructor(seed: string | number) {
        ...
        // Fisher-Yates over the 12 gradients, then duplicate to 512 entries.
    ...
    /** 2D simplex noise, roughly in [-1, 1]. Same contract as the lib. */
