...
declare const sandkit: { api: any; engine?: any };

const rawApi = (): any => {
  try { return (sandkit as any).api; } catch { return null; }
};

const rawEngine = (): any => {
  try { return (sandkit as any).engine; } catch { return null; }
};
...
function resolveRef(id: string): string | number | null {
  ...
  for (const c of candidates) {
    try {
      ...
      if (typeof t === "number" && isFinite(t)) { found = t; break; }
    } catch { /* */ }
  }
...
function spawnMaterializeLight(cellX: number, cellY: number, radius: number): void {
  ...
  try {
    ...
    if (typeof lights?.createAtWorld === "function") {
      let cellSize = 16;
      try {
  ...
  // Recompute local shadows after terrain appears
  try {
    const eng = rawEngine();
    const state = eng?.state;
    const refresh = eng?.api?.shadows?.refreshRadius;
    if (typeof refresh === "function" && state) {
      refresh(state, cellX, cellY, Math.round(radius) + 5);
    }
  } catch { /* shadows optional */ }
}
...
function placeOne(
  ...
  for (const t of [terrains, apiTerrains]) {
    if (!t) continue;
    try { t.createAtCell?.(x, y, ref); wrote = true; } catch { /* */ }
    try { t.replaceAtCell?.(x, y, ref); wrote = true; } catch { /* */ }
  }
  return wrote;
}
